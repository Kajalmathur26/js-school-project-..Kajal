...
### Task 4:
- Introduced TypeScript and static typing for safer code.
- Initialized project with tsconfig.json in strict mode.
- Converted script.js → src/index.ts and defined interfaces/types for event data.
- Modularized code using ES modules: fetcher.ts, renderer.ts, modal.ts, etc.
- Added build scripts to compile TypeScript → JavaScript into a dist/ folder.
- Verified the app works in the browser with the compiled JS.
